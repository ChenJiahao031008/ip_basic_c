...
public:
    ...
    cv::Mat JointBilateralFilterInpaintingOMP(cv::Mat colorImage, cv::Mat depthImage,
        vector<vector<int>> &inpainting_index, int kernel_size, double sigma_pos, double sigma_col,
        double sigma_depth);

    cv::Mat RefineInpaintingArea(cv::Mat color, cv::Mat depthImage, vector<vector<int>> inpainting_index);
